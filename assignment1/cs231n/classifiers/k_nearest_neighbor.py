...
class KNearestNeighbor(object):
  ...
  def predict_labels(self, dists, k=1):
    ...
    - y: A numpy array of shape (num_test,) containing predicted labels for the
      test data, where y[i] is the predicted label for the test point X[i].  
    """
    num_test = dists.shape[0] #i.e. 500
    y_pred = np.zeros(num_test)
    distsIdx = np.argsort(dists, axis = 1) #returns sorted indexes
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      Idx = distsIdx[i,:k] #chooses k nearest neighbours for each sample
      closest_y = self.y_train[Idx] #returns label for k nearest neighbours 
      #########################################################################
      # TODO:                                                                 #
